...
export const createPrecompositeImage = (
    ...
    return new Promise((resolve, reject) => {
        // console.log(`[createPrecompositeImage] Starting for clothing: ${clothingDataUrl.substring(0, 30)}..., print: ${printDataUrl ? printDataUrl.substring(0, 30) + '...' : 'none'}`);

        if (!clothingDataUrl) {
            // console.warn("[createPrecompositeImage] No clothingDataUrl provided, resolving null.");
            return resolve(null);
        }
        ...
        clothingImg.onload = () => {
            // console.log("[createPrecompositeImage] Clothing image loaded.");
            const canvas = document.createElement('canvas');
            const ctx = canvas.getContext('2d');
            if (!ctx) {
                console.error("[createPrecompositeImage] Could not get canvas context.");
            ...
            if (!printDataUrl || !mask) {
                // console.log("[createPrecompositeImage] No print or mask, resolving clothing only.");
            ...
            printImg.onload = () => {
                // console.log("[createPrecompositeImage] Print image loaded, compositing.");
                ...
                ctx.restore();
                // console.log("[createPrecompositeImage] Compositing complete, resolving image.");
                resolve(canvas.toDataURL('image/png'));
            };
            printImg.onerror = (e) => {
                console.error("[createPrecompositeImage] Failed to load print image.", e);
                reject(new Error("Failed to load print image."));
            };
            printImg.src = printDataUrl;
        };

        clothingImg.onerror = (e) => {
            console.error("[createPrecompositeImage] Failed to load clothing image.", e);
